...
class PlayerObject(object):
    def __init__(self, moveSetKey, dashKey, numVertices, position):
        ...
        self.image = GFXDrawShape(self.numSides, self.size, self.color)
        self.image = pygame.transform.smoothscale(pygame.image.load('Images\Hexagon.png'), (self.size, self.size))
        # self.size = (30, 30)
    ...
    def update(self, dt, TRAILS):
        ...
        self.image = GFXDrawShape(self.numSides, self.size, self.color, self.angle)
        ...
        if TimeIt(self.trailTimer, self.trailStart) and movement:
            ...
            TRAILS.append(self.trailType(self.position, self.angle, self.trailDuration, 
                                        totalAngleChange, PolyInfo=(self.numSides, self.size, self.color), 
    ...
    def returnScreenRect(self):
        return pygame.Rect((self.position), (2*self.size, 2*self.size))
